...
function resolveDefault(value: string | undefined, defaultValue: string): string[] | undefined {
...
export default function AccordionIndexed({
  ...
  return (
    <div className={cn("mx-auto w-full max-w-lg", className)}>
      <Accordion
        defaultValue={resolveDefault(value, defaultValue)}
        value={value ? [value] : undefined}
        onValueChange={(next) => {
          if (Array.isArray(next)) onValueChange?.(next[0] ?? "")
        }}
